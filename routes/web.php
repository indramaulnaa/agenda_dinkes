...
Route::middleware([CheckPegawaiPin::class])->group(function () {
    ...
    Route::get('/', function () {
        $rooms = ['Aula Utama Dinas Kesehatan Lt 2', 'Kepala Dinas Kesehatan Lt 1', 'Sekertaris Dinas Kesehatan Lt 1'];
        $roomStatus = [];
...
Route::middleware(['auth'])->group(function () {
    ...
    // VIEW: Menggunakan MeetingRoomController
    Route::get('/meeting-room', [MeetingRoomController::class, 'index'])->name('meeting-room.index');
    
    // ACTION (Simpan/Update/Hapus): Menggunakan AgendaController (karena logika ada di sana)
    Route::post('/meeting-room', [AgendaController::class, 'store'])->name('meeting-room.store');
    Route::put('/meeting-room/{id}', [AgendaController::class, 'update'])->name('meeting-room.update');
    ...
    // Pisahkan route untuk update password agar lebih spesifik
    ...
    Route::middleware([CheckSuperAdmin::class])->group(function () {
        ...
        // Update Config WA
        Route::put('/settings/update-config', [SettingsController::class, 'updateConfig'])->name('settings.updateConfig');
    });
});